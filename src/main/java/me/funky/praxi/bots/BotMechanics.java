...
public class BotMechanics extends BukkitRunnable
{
    ...
    private void startupBotMechanics() {
        ...
        if (this.difficulty == me.funky.praxi.bots.Bot.BotDifficulty.EASY) {
            this.attackRange *= 0.8;
            this.swingRangeModifier = -0.5;
        }
        else if (this.difficulty == me.funky.praxi.bots.Bot.BotDifficulty.HARD) {
            this.attackRange *= 2.0;
            this.swingRangeModifier = 2.0;
        }
        else if (this.difficulty == me.funky.praxi.bots.Bot.BotDifficulty.EXPERT) {
            this.attackRange *= 2.6;
            this.swingRangeModifier = 3.0;
            delay = 1;
        }
        this.runTaskTimerAsynchronously((Plugin) Praxi.get(), 60L, (long)delay);
    }
                    ...
                    new BukkitRunnable() {
                        public void run() {
                            if (BotMechanics.this.Bot.getBukkitEntity() != null) {
                                ...
                                }.runTaskLater((Plugin)Praxi.get(), (long)(BotMechanics.this.random.nextInt(4) + 2));
                            }
                        }
                    }.runTaskLater((Plugin)Praxi.get(), 35L);
                }
            }.runTaskLater((Plugin)Praxi.get(), (long)(this.random.nextInt(2) + 1));
            return true;
        }
        return false;
    }
            ...
            new BukkitRunnable() {
                public void run() {
                    ...
                    if (!BotMechanics.this.Bot.getBukkitEntity().isDead()) {
                        ...
                        }.runTaskLater((Plugin)Praxi.get(), (long)(BotMechanics.this.random.nextInt(1) + 1));
                    }
                }
            }.runTaskLater((Plugin)Praxi.get(), (long)(this.random.nextInt(1) + 1));
            return true;
        }
        return false;
    }
                ...
                public void run() {
                    if (BotMechanics.this.Bot.getNpc() != null && BotMechanics.this.Bot.isSpawned() && BotMechanics.this.Bot.getNpc().getNavigator() != null) {
                        ...
                        if (this.counter == 0 || Math.abs(BotMechanics.this.Bot.getBukkitEntity().getLocation().getPitch() - 90.0f) < 50.0f) {
                            ...
                            }.runTaskTimer((Plugin)Praxi.get(), 1L, 1L);
                            BotMechanics.this.Bot.getBukkitEntity().setItemInHand(new ItemStack(Material.AIR));
                            ...
                            }.runTaskLater((Plugin)Praxi.get(), (long)(BotMechanics.this.random.nextInt(12) + 8));
                        }
                    }
                    else {
                        this.cancel();
