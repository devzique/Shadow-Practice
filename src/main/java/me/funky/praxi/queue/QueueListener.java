		...
		if (event.getItem() != null && (event.getAction() == Action.RIGHT_CLICK_AIR ||
			...
			if (hotbarItem != null) {
				...
				if (hotbarItem == HotbarItem.QUEUE_JOIN_RANKED) {
                    if (profile.getWins() < Praxi.get().getSettingsConfig().getInteger("RANKED.REQUIRED-WINS")) {
                        event.getPlayer().sendMessage(CC.translate(Praxi.get().getMainConfig().getString("RANKED.ERROR-MESSAGE"))
                            .replace("{wins}", String.valueOf(Praxi.get().getMainConfig().getInteger("RANKED.REQUIRED-WINS") - profile.getWins())));
                        return;
                    }
					if (!profile.isBusy()) {
