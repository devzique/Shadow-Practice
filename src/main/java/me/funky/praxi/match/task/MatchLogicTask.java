...
public class MatchLogicTask extends BukkitRunnable {
    ...
    public void run() {
        ...
        if (match.getState() == MatchState.STARTING_ROUND) {
            if (match.getKit().getGameRules().isSumo() || match.getKit().getGameRules().isBedfight() || match.getKit().getGameRules().isBridge() || match.getKit().getGameRules().isStickFight()) {
            ...
            if (nextAction == 0) {
				...
				if (match.getKit().getGameRules().isSumo() || match.getKit().getGameRules().isBedfight() || match.getKit().getGameRules().isStickFight() || match.getKit().getGameRules().isBridge()) {
                    for (GameParticipant<MatchGamePlayer> gameParticipant : match.getParticipants()) {
                        for (GamePlayer gamePlayer : gameParticipant.getPlayers()) {
