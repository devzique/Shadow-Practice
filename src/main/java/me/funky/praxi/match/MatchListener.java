...
public class MatchListener implements Listener {
	...
	private final HashMap<UUID, Integer> portalEntryCount = new HashMap<>();
        ...
        if (event.getEntity() instanceof Arrow) {
            if (event.getEntity().getShooter() instanceof Player) {
                ...
                if (shooterData.getState() == ProfileState.FIGHTING ) {
                    ...
                    if (match.getKit().getGameRules().isBridge())
                    // Remove the arrow after 3 seconds
	...
	public void onPlayerItemConsumeEvent(PlayerItemConsumeEvent event) {
        ...
        if (Praxi.get().getSettingsConfig().getBoolean("MATCH.REMOVE_POTION_BOTTLE")) {
