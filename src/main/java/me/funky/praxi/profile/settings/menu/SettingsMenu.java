...
import me.funky.praxi.util.assemble.AssembleListener;
    ...
    private class SettingsButton extends Button {
        ...
        public ItemStack getButtonItem(Player player) {
            ...
            switch (this.settings) {
				...
				case THEME: {
                    ...
                    lore.add(profile.getOptions().theme().equals(Themes.ORANGE) ? " &a■ &fOrange" : " &8■ &fOrange");
                    lore.add("");
                    if (player.hasPermission("shadow.donator.themes")) {
                        lore.add("&aClick to change.");
                    } else {
        ...
        public void clicked(Player player, ClickType clickType) {
            ...
            switch (this.settings) {
				...
				case THEME: {
                    if (!player.hasPermission("shadow.donor.themes")) {
        ...
        public SettingsButton(Settings settings) {
		 ...
		 public void showScoreboard(Profile profile, boolean b) {
            ...
            if (profile.getOptions().showScoreboard()) {
                if (Praxi.get().getAssemble().isCallEvents()) {
                ...
                Praxi.get().getAssemble().getBoards().put(profile.getPlayer().getUniqueId(),
                        new AssembleBoard(profile.getPlayer(), Praxi.get().getAssemble()));
            } else {
                if (Praxi.get().getAssemble().isCallEvents()) {
                ...
                Praxi.get().getAssemble().getBoards().get(profile.getPlayer().getUniqueId()).getObjective().unregister();
                Praxi.get().getAssemble().getBoards().remove(profile.getPlayer().getUniqueId());
            }
        }
    }
}
