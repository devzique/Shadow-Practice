...
/* TODO: Recode Tournaments system */
public class Tournament {
    ...
    public void start() {
        ...
        while (iterator.hasNext()) {
            final Party player = iterator.next();
            if (!iterator.hasNext()) {
            ...
            final Party other = iterator.next();
            final Arena arena = Arena.getRandomArena(this.kit);
            final GameParticipant<MatchGamePlayer> teamA = new GameParticipant<>(new MatchGamePlayer(player.getLeader().getPlayer().getUniqueId(), player.getLeader().getPlayer().getDisplayName()));
            final GameParticipant<MatchGamePlayer> teamB = new GameParticipant<>(new MatchGamePlayer(other.getLeader().getPlayer().getUniqueId(), other.getLeader().getPlayer().getDisplayName()));
            final TournamentMatch tournamentMatch = new TournamentMatch(teamA, teamB, this.getKit(), arena);
            for (final UUID uuid : player.getPlayers()) {
                Player player2 = Bukkit.getPlayer(uuid);
                final Profile otherData = Profile.getByUuid(player2.getUniqueId());
            ...
            for (final UUID uuid : other.getPlayers()) {
                Player player2 = Bukkit.getPlayer(uuid);
                final Profile otherData = Profile.getByUuid(player2.getUniqueId());
                otherData.setState(ProfileState.FIGHTING);
                otherData.setMatch(tournamentMatch);
